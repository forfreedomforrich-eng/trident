...
* *On-premise ONTAP* - Trident supports Kerberos encryption over NFSv3 and  connections from Red Hat OpenShift and upstream Kubernetes clusters to on-premise ONTAP volumes.
...
* Ensure that any  volumes you use with Kerberos encryption are configured correctly. Refer to the NetApp  Domain Configuration section (page 13) of the https://www.netapp.com/media/16398-tr-3580.pdf[NetApp  Enhancements and Best Practices Guide^]. 
...
.Access protocols
Configure the export policy with NFS, NFSv3, and  access protocols.
