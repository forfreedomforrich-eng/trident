...
The following example imports the `ontap-san-managed` FlexVol volume that is present on the `ontap_san_default` backend:
----
tridentctl import volume BackendForiSCSIEco toimport/lunFoo -f import1.yaml
...
NOTE: Post first LUN inside toimport is imported, the flexvol is renamed. In the subsequent import LUNs(LUNs inside same flexvol), the import statement should use the newly renamed flexvol name.
For example, if the second LUN (lun2) previously inside toimport which would be changed say trident_lun_pool_xyz import statement will change to:

----
trident import volume BackendForiSCSIEco trident_lun_pool_xyz/lun2 -f import2.yaml
----
