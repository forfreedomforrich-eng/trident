...
The following show an example of a managed volume and an unmanaged volume import for the `ontap-san-economy` backend.
...
The following example imports the `vol-managed-saneco` san-economy volume that is present on the LUN `toimport/lun1`:
----
tridentctl import volume vol-managed-saneco toimport/lun1 -f import1.yaml
----

NOTE: After the first LUN inside toimport is imported, the flexvol is renamed. In the subsequent import, the LUNs(LUNs inside same flexvol) import statement should use the newly renamed flexvol name.
For example, if the second LUN (lun2) previously inside toimport which would be changed (e.g, trident_lun_pool_xyz), the import statement will change to:

----
trident import volume vol-managed-saneco trident_lun_pool_xyz/lun2 -f import2.yaml
----
LUNs in the ontap-san-economy backend are imported one LUN at a time, which can be automated using a script.

--
.Unmanaged volume
--
For unmanaged import, the flexVol should be named in this format: `"trident_lun_pool_" + backend_config.get("storagePrefix", "trident_").rstrip("_") + "_any_random_string"` and the LUN should be named lun0 to adhere to trident naming conventions
If a storagePrefix is `xyz`, the flexvol name for unmanaged import looks like this: `trident_lun_pool_xyz_randomstring`
Full path to the LUN is: trident_lun_pool_xyz_randomstring/lun0 
--
