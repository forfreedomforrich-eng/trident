...
----
+
NOTE: Ensure that the namespace intended for the application restore exists on the destination cluster. ##If the AppVault CR does not exist on the destination cluster, create it by following the steps in link:trident-protect-appvault-custom-resources.html[Use Trident Protect AppVault objects to manage buckets].##
